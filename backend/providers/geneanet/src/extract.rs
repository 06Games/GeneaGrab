...
fn parse_image_api(base_url: &Url, api_json: &str) -> Result<Vec<Image>, ProviderError> {
    let json_array: Vec<serde_json::Value> =
        serde_json::from_str(api_json).map_err(|e| ProviderError::ParsingError(e.to_string()))?;

    let mut images = Vec::new();
    for item in json_array {
        if let Some(page_num) = item.get("page").and_then(serde_json::Value::as_u64) {
            let page_num =
                u32::try_from(page_num).map_err(|e| ProviderError::ParsingError(e.to_string()))?;
...
pub async fn extract_registry_internal(
    ...
    let parsed_view_url: Url = Url::parse(&view_url)
        .map_err(|e| ProviderError::InvalidField(format!("Invalid view URL: {e}")))?;
    ...
    let html = fetcher.fetch(view_url.into()).await?;
    ...
    let api_json = fetcher.fetch(api_url.into()).await?;
