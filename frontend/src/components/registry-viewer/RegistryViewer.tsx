import { createSignal, createEffect, onCleanup, onMount, createResource, Show, ErrorBoundary } from "solid-js";
...
export const RegistryViewer = (props: RegistryViewerProps) => {
  ...
  const [registryMeta] = createResource(() => props.registryId, api.getRegistryMeta);
  const [eventRows] = createResource(() => props.registryId, api.getEventRows);
  const [imageMeta, { mutate: mutateImageMeta }] = createResource(
    () => isDetachedMode ? false : { regId: props.registryId, imgId: currentImage() },
  ...
  createEffect(() => {
    if (!isDetachedMode) {
      navigate(`/registry/${props.registryId}/${currentImage()}`, { replace: true });
    }
  });
  ...
  const { isDetached, setIsDetached, detach, closeSelf, sendMessage } = useDetachedWindow<SyncMessage>({
    id: `index-${props.registryId}`,
    title: `Index - ${props.registryId}`,
  ...
  const registryActions = {
    onSaveImageMeta: async (meta: Partial<UserImageMeta>) => {
      await api.saveImageMeta(props.registryId, currentImage(), meta);
      mutateImageMeta(prev => prev ? { ...prev, ...meta } : prev);
    },
    onSaveAct: async (event: EventDetail) => {
      await api.saveAct(event);
      // Optional: Refresh eventRows if needed -> api.getEventRows(props.registryId).then(mutateEventRows)
    },
  ...
  return (
    <RegistryActionsProvider {...registryActions}>
      <Show when={!registryMeta.error} fallback={
        <div class="flex flex-col items-center justify-center w-full h-full text-danger bg-app">
          ...
          <p class="text-[15px] font-medium">Failed to load registry</p>
        </div>
      }>
        <Show when={registryMeta()} fallback={
          <div class="flex flex-col items-center justify-center w-full h-full text-dim bg-app gap-3">
            ...
            <span class="text-[14px]">Loading registry data...</span>
          </div>
        }>
          {(regMeta) => {
            if (isDetachedMode) {
              ...
              <div class="flex flex-col w-screen h-screen overflow-hidden bg-app text-main select-none antialiased" style={{ "font-family": "'Outfit', 'Helvetica Neue', system-ui, sans-serif" }}>
                ...
                <div class="flex flex-1 min-h-0 overflow-hidden">
                  <main class="flex-1 flex flex-col min-w-0 overflow-hidden">
                    <MainViewer currentImage={currentImage()} totalImages={regMeta().total_images} onImageChange={setCurrentImage} registryId={props.registryId} />
                    ...
                    <ThumbnailBar height={thumbnailHeight()} totalImages={regMeta().total_images} currentImage={currentImage()} onImageChange={setCurrentImage} registryId={props.registryId} />
                  </main>
                ...
                <footer class="flex-shrink-0 flex items-center justify-between px-4 h-6 bg-panel border-t border-subtle" role="status">
                  <div class="flex items-center gap-4">
                    <span class="text-[11px] text-dim">{currentImage()} / {regMeta().total_images}</span>
                    <span class="text-[11px] text-dim">{t("registryViewer.viewsAndActs", { views: regMeta().total_images, acts: eventRows()?.length || 0 })}</span>
                  </div>
