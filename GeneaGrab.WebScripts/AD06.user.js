...
if (window.location.href.startsWith("http://www.basesdocumentaires-cg06.fr/archives/ImageZoomViewerEC")) {
    ...
    $buttons = $closeBtn.parent();
  	...
  	$('<a class="lien" href="">Copier les arguments de la page<input readonly type="text" style="display: none;" /></a>').appendTo($buttons).click(function(event) {
    ...
    $openIn = $('<a class="lien" href="geneagrab:registry?url=' + window.location.href + '">Ouvrir dans GeneaGrab</a>').appendTo($buttons).hover(updateLink).mousemove(updateLink).click(updateLink);
    function updateLink() {
        $openIn.attr('href', 'geneagrab:registry?url=' + window.location.href + "&page=" + $pageDiv.text());
    }


  	$buttons.children().each(function(){$(this).css("display", "block"); });
